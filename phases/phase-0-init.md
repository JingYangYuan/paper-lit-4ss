...
由 `SKILL.md` 在每次执行时首先加载。处理目录创建、搜索日志初始化、论文清单初始化和模式参数解析。

---

## 0. 顾问派发闸门

进入 Phase 0/1 前，必须根据 `master/agent-orchestration.md` 创建 `paper-workspace/_logs/agents/lit-[YYYY-MM-DD]/agent-brief.md`。正式检索前派发 `agents/search-strategy-consultant.md`，让其先给出检索词、来源组合和补洞策略；若为完整地图、定向综述或假设模式，同时预派发 `agents/theory-map-consultant.md` 做理论谱系预判。不能并行时按顺序复核并记录 `sequential-review`。
...
python3 scripts/literature_registry.py init --workspace "${OUTPUT_ROOT}"
```
...
```bash
python3 scripts/literature_registry.py register \
...
- `浏览器控制正常`：浏览器控制后端可列标签页/新建标签页/导航且检索页可达（ZCode 内置 browser-use；OMP pi-chrome 按 [pi-chrome-browser.md](references/pi-chrome-browser.md) §3 四项验收通过）；继续网页操纵。
