...
进入假设推导前，必须派发 `agents/hypothesis-bridge-consultant.md`，检查空白、理论框架、机制链条和假设表述是否连贯。顾问意见写入 `paper-workspace/_logs/agents/lit-[YYYY-MM-DD]/hypothesis-bridge-consultant.md`，并由主流程更新 `agent-synthesis-lit-[YYYY-MM-DD].md` 后再输出假设。
...
1. **加载路由索引**：先 Read `references/theory-frameworks.md`，根据研究主题关键词匹配路由表，确定 1-3 个最相关学科
2. **按需加载学科框架**：根据路由结果，Read 对应的 `design/frame/theory-frameworks-[discipline].md` 文件（路径：`design/frame/`）
...
路由索引及加载指令详见 [theory-frameworks.md](references/theory-frameworks.md)。
...
详见 [gap-to-hypothesis.md](references/gap-to-hypothesis.md)。
