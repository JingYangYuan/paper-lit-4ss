...
> **模式E执行轨道（唯一）**：使用 ZCode 内置浏览器控制走 **kns8s 闭环轨道**（检索→分析摘要→选择下载全文一体化），完整协议见 [cnki-kns8s-closed-loop.md](references/cnki-kns8s-closed-loop.md)（经验来源 `~/.zcode/skills/cnki-skill`，已实跑验证；下载器 `scripts/cnki/kns8s-download.sh`）。浏览器控制不可用时记录 `浏览器控制不可用` 并停止 CNKI 阶段。
