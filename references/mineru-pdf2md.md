...
> 2026-09-05 由 mineru-pdf2md skill 合并入本模块：脚本 `scripts/mineru/pdf2md.py` 与 Token 配置 `scripts/mineru/.env.kie` 均为模块内部资产，lit 流程**不得引用外部 skill 路径**。
...
python3 scripts/mineru/pdf2md.py \
...
python3 scripts/mineru/pdf2md.py <PDF目录> --split-long --output <输出目录>

# Token 失效且文件 ≤10MB/≤20页 时
python3 scripts/mineru/pdf2md.py <单文件.pdf> --agent --output <输出目录>
```

- Token 来源优先级：`--token` 参数 → 目标目录/父目录 `.env.kie` → `~/.config/mineru/.env.kie` 等用户级 → `scripts/mineru/.env.kie`。
