...
验证码与下载约定：验证码出现时停止自动化并请用户在可见浏览器中手动拖动完成（OMP 下用户切到 `Pi Session:` 分组标签）；PDF 下载不走浏览器下载管线（必弹 Save-As），使用 `scripts/cnki/kns8s-download.sh`（Cookie + curl）免弹窗下载；Cookie 获取 OMP 走回环 sink（`scripts/cnki/cookie_sink.py`），不得经对话回传。完整协议见 [cnki-kns8s-closed-loop.md](cnki-kns8s-closed-loop.md)。
...
test -f references/pi-chrome-browser.md
test -f scripts/cnki/kns8s-download.sh
test -f scripts/cnki/cookie_sink.py
python3 scripts/cnki/cookie_sink.py --help
```
